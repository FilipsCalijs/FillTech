...
const CREATE_URL = 'https://api.wavespeed.ai/api/v3/wavespeed-ai/audio-vocal-isolator';
...
router.post('/', (req, res, next) => {
  ...
  try {
    ...
    const { genId, imageUrl: audioUrl } = await runWavespeed({
      ...
      body:      { audio: dataUri },
      userUid,
      toolType:  'vocal-isolator',
      r2Folder:  'vocal-isolator',
    });

    res.json({ success: true, audioUrl, generationId: genId });
  } catch (err) {
    console.error('[vocal-isolator]', err);
