...
const CREATE_URL = 'https://api.wavespeed.ai/api/v3/wavespeed-ai/ai-photo-colorizer';
...
router.post('/', (req, res, next) => {
  ...
  try {
    ...
    const { genId, imageUrl } = await runWavespeed({
      ...
      body: { image: dataUri },
      userUid,
      toolType:  'photo-colorize',
      r2Folder:  'photo-colorize',
    });

    res.json({ success: true, imageUrl, generationId: genId });
  } catch (err) {
    console.error('[photo-colorize]', err);
