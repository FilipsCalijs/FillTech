...
const Avatar = ({ src, name }) => {
  ...
  return (
    <div className="w-9 h-9 rounded-full bg-muted flex items-center justify-center shrink-0">
      <span className="text-sm font-semibold text-muted-foreground">{name?.[0]?.toUpperCase() || '?'}</span>
    </div>
  );
};
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <Typography variant="label" weight="semibold" className="block">{comment.display_name}</Typography>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={handleSave} disabled={!draft.trim()}>
                <Check size={14} className="mr-1" /> {tc('actions.save')}
              </Button>
              <Button size="sm" variant="ghost" onClick={handleCancel}>
                <X size={14} className="mr-1" /> {tc('actions.cancel')}
              </Button>
            </div>
          </div>
        ) : (
          <div className="flex items-start justify-between gap-2">
            <Typography variant="body2" className="block whitespace-pre-wrap break-words">{comment.content}</Typography>
            {canModify && (
              <div className="flex gap-1 shrink-0">
                <button onClick={() => setEditing(true)} className="p-1.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
                  <Pencil size={13} />
                </button>
                <button onClick={() => onDelete(comment.id)} className="p-1.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-destructive">
                  <Trash2 size={13} />
                </button>
              </div>
            )}
...
const Comments = ({ postId }) => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const { data } = await axios.post(`${API}/api/comments`, { postId, content: text.trim() }, { headers: headers() });
  ...
  const handleEdit = async (id, content) => {
    try {
      const { data } = await axios.put(`${API}/api/comments/${id}`, { content }, { headers: headers() });
  ...
  return (
    <section className="mt-12 pt-8 border-t border-border">
      ...
      {loading ? (
        <Typography variant="body2" color="muted" className="block">{tc('loading')}</Typography>
      ) : comments.length === 0 ? (
        <Typography variant="body2" color="muted" className="block mb-6">{t('comments.empty')}</Typography>
      ) : (
        <div className="flex flex-col gap-5 mb-8">
          {comments.map(comment => (
            <CommentItem key={comment.id} comment={comment} currentUid={currentUid} isAdmin={isAdmin} onEdit={handleEdit} onDelete={handleDelete} />
          ))}
        </div>
      )}
      ...
      {userLoggedIn ? (
        <div className="flex gap-3">
          ...
          <div className="flex-1 flex flex-col gap-2">
            <textarea
              className="w-full rounded-lg border border-border bg-background p-3 text-sm resize-none focus:outline-none focus:ring-1 focus:ring-ring"
              rows={3} placeholder={t('comments.placeholder')} value={text} onChange={e => setText(e.target.value)}
            />
            <div className="flex justify-end">
              <Button size="sm" onClick={handleSubmit} disabled={submitting || !text.trim()} isLoading={submitting}>
                {t('comments.submit')}
              </Button>
            </div>
          </div>
